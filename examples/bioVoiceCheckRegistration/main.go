...
func main() {
	...
	registration, _ := utils.AskForInput("Check biovoice registration for phone number [6100000000]: ", "6100000000")
	bioVoiceResponse, err := twizo.BioVoiceCheckRegistration(registration)
	if aErr, ok := err.(*twizo.APIError); ok && aErr.NotFound() {
		fmt.Printf("Biovoice registration for [%s] not found.\n", registration)
		os.Exit(1)
	}
	if err != nil {
		panic(err)
